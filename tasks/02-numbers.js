...
function intWithinBounds(n, lower, upper) {
  const num = (Number.isInteger(n) && n > lower && n < upper) ? true : false;
  return num;
}
...
function tempConversion(celsius) {
  const kelvin = (celsius + 273.15).toFixed(2),
    ...
    temperature = (kelvin < 0) ? "Invalid" : [fahrenheit, kelvin];
  return temperature;
}
...
function countOnes(i) {
  const newI = '' + parseInt(i).toString(2);
  let count = 0;
